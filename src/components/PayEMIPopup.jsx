...
export default function PayEMIPopup({ open, setOpen, loanId, onPaid }) {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Failed to process EMI payment');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="modal-overlay">
      ...
      <div className="modal modal-sm" style={{ zIndex: 11 }}>
        ...
        <div className="modal-sub">Confirm payment for the current upcoming installment.</div>
        ...
        {fetching ? (
          ...
          <>
            <div style={{ background: 'var(--bg3)', borderRadius: 'var(--r)', padding: '16px', marginBottom: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                <div>
                  <div style={{ fontSize: '11px', color: 'var(--text3)', textTransform: 'uppercase', fontWeight: 600 }}>Loan</div>
                  <div style={{ fontSize: '14px', fontWeight: 700 }}>{loanData.bankName}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: '11px', color: 'var(--text3)', textTransform: 'uppercase', fontWeight: 600 }}>Inst. #</div>
                  <div style={{ fontSize: '14px', fontWeight: 700 }}>{nextInstallment.installmentNo}</div>
                </div>
              </div>

              <div style={{ padding: '12px', background: 'var(--bg4)', borderRadius: 'var(--r2)', marginBottom: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--text2)' }}>Principal Component</span>
                  <span style={{ fontSize: '12px', fontWeight: 600 }}>{formatAmount(nextInstallment.principalComponent, currency, decimalPlaces)}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: '12px', color: 'var(--text2)' }}>Interest Component</span>
                  <span style={{ fontSize: '12px', fontWeight: 600 }}>{formatAmount(nextInstallment.interestComponent, currency, decimalPlaces)}</span>
                </div>
              </div>

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <div>
                  <div style={{ fontSize: '11px', color: 'var(--text3)', textTransform: 'uppercase', fontWeight: 600 }}>Due Date</div>
                  <div style={{ fontSize: '13px', fontWeight: 500 }}>{format(new Date(nextInstallment.dueDate), 'dd MMM yyyy')}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: '11px', color: 'var(--text3)', textTransform: 'uppercase', fontWeight: 600 }}>Total EMI</div>
                  <div style={{ fontSize: '20px', fontWeight: 800, color: 'var(--accent)', fontFamily: 'var(--mono)' }}>
              ...
              >
                ...
                {accounts.map((a) => (
                  <option key={a._id} value={a._id}>
                    {a.name} (Bal: {formatAmount(a.balance, currency, decimalPlaces)})
                  </option>
                ))}
              </select>
            </div>
