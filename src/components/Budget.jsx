    ...
    <div className="page-body p-6">
      ...
      <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard
          ...
          color="amber"
          icon="⚠️"
          subtext={stats.overBudgetCount > 0 ? 'Review high-spend areas' : 'All within limits'}
        />
      </div>

      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-[#EEF0F8]">Monthly Budgets</h2>
          <p className="text-xs text-[#8892B0]">Track spending across your active categories.</p>
        </div>
        <button
      ...
      {loading && budgets.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm font-semibold text-[#EEF0F8]">No budgets defined yet</p>
          <p className="mt-1 text-xs text-[#8892B0]">Click to set your first spending limit</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          >
            ...
            <div className="mt-1 text-[10px] text-[#4A5578]">Set a new category limit</div>
          </div>
        </div>
      )}
...
const KpiCard = ({ label, value, color, icon, subtext }) => {
  ...
  return (
    <div className={cn(
      ...
      <div className="text-[10px] font-bold uppercase tracking-widest text-[#4A5578]">{label}</div>
      <div className="mt-1 font-mono text-2xl font-bold tracking-tight text-[#EEF0F8]">{value}</div>
      {subtext && <div className="mt-2 text-[10px] text-[#8892B0]">{subtext}</div>}
    </div>
  );
};
...
const BudgetCard = ({ budget, onEdit, currency }) => {
  ...
  const barColor = isOver ? 'bg-[#FF6B6B]' : isWarning ? 'bg-[#F5A623]' : 'bg-[#2DD4A0]';
  const textColor = isOver ? 'text-[#FF6B6B]' : isWarning ? 'text-[#F5A623]' : 'text-[#2DD4A0]';
    ...
    <div className={cn(
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-[#EEF0F8]">{budget.category?.name}</h3>
            <p className={cn("text-[10px] uppercase font-bold tracking-wider", isOver ? "text-[#FF6B6B]" : "text-[#8892B0]")}>
