...
import { 
  ArrowLeft, X, ShieldCheck, Target, TrendingUp, TrendingDown,
  Info, Activity, Zap, Wallet, BarChart3, ArrowRight,
  PieChart as PieChartIcon, Gem, Landmark, Loader2, RefreshCw
} from 'lucide-react';
import { 
...
const FinancialAnalysis = () => {
  ...
  const stabilityAdvice = useTypewriter(
    ...
    ) : "Initializing neural insights...", 30, 800
  );

  const marketInsight = useTypewriter(
    "Based on current acquisition velocity and zero-liability status, your net worth structure is poised for high-magnitude expansion.", 25, 1500
  );
  ...
  useEffect(() => {
    ...
    systemMessages.forEach((msg, i) => {
      setTimeout(() => {
        setLogs(prev => [...prev.slice(-3), { id: i, msg, time: new Date().toLocaleTimeString() }]);
      }, i * 800);
    });
  }, []);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#06080F] gap-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <h2 className="text-2xl font-black text-white tracking-[0.3em] uppercase mb-1">Architect AI</h2>
          <p className="text-accent text-[10px] font-black uppercase tracking-widest mb-8">Booting Neural Engine</p>
          ...
          <div className="space-y-2 text-center h-20">
            {logs.map((log, i) => (
              <div key={log.id} className="text-[10px] font-mono text-text3 flex items-center gap-3 animate-in fade-in slide-in-from-bottom-2 duration-500">
  ...
  const { liquidAssets, investments, physicalAssets, totalAssets, totalLiabilities, breakdown } = overview;
  ...
  const laRatio = totalAssets > 0 ? (totalLiabilities / totalAssets).toFixed(2) : "0.00";
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <h1 className="text-4xl font-extrabold text-text tracking-tight">Financial Analysis</h1>
          </div>
        </div>
        ...
        <div className="lg:col-span-4">
          <div className="account-card h-full !p-8 flex flex-col items-center text-center relative overflow-hidden group">
            ...
            <div className="relative mb-8 flex items-center justify-center p-2">
              ...
              <svg className="w-40 h-40 transform -rotate-90 origin-center overflow-visible relative z-10" viewBox="0 0 160 160">
                <circle 
                  cx="80" 
                  cy="80" 
                  r="64" 
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-[11px] font-bold text-text3 uppercase mt-1">Health Score</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full relative z-10">
            <div className="account-card p-8 group hover:border-accent/30 transition-colors cursor-default relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-6">
                ...
                <div className={`text-[10px] font-bold px-3 py-1 rounded-full ${parseFloat(laRatio) < 0.2 ? 'bg-green/10 text-green border border-green/20' : 'bg-red/10 text-red border border-red/20'}`}>
                  {parseFloat(laRatio) < 0.2 ? 'OPTIMAL' : 'HIGH LEVERAGE'}
                </div>
              </div>
              <div className="text-[11px] font-black text-text3 uppercase tracking-[0.2em] mb-2">Leverage Ratio</div>
              <div className="text-5xl font-bold text-white font-mono tracking-tighter">{laRatio}x</div>
              <p className="text-[10px] text-text3 mt-4 leading-relaxed italic opacity-60 font-medium">Debt-to-Asset architecture efficiency</p>
            </div>
            ...
            <div className="account-card p-8 group hover:border-purple-500/30 transition-colors cursor-default">
              ...
              <div className="text-[11px] font-black text-text3 uppercase tracking-[0.2em] mb-2">Liquidity Coverage</div>
              <div className="text-5xl font-bold text-white font-mono tracking-tighter">
                {((liquidAssets / (totalLiabilities || 1)) * 100).toFixed(0)}%
              </div>
              <p className="text-[10px] text-text3 mt-4 leading-relaxed italic opacity-60 font-medium">Emergency liability coverage capacity</p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4">
          <div className="account-card h-full !p-8">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h4 className="font-bold text-text uppercase text-xs tracking-widest">Asset Allocation</h4>
            </div>
              ...
              {allocationData.map((d, i) => (
                <div key={i} className="flex items-center justify-between p-3 bg-white/[0.02] rounded-xl border border-white/5">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[12px] font-bold text-text3 uppercase tracking-wider">{d.name}</span>
                  </div>
                  <span className="text-sm font-bold text-white">{((d.value / totalAssets) * 100).toFixed(0)}%</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="account-card h-full !p-8">
            <div className="flex items-center justify-between mb-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-bold text-text text-xl">Interest Efficiency Audit</h4>
                  <p className="text-[10px] text-text3 uppercase font-bold tracking-[0.15em] mt-1">Capital Erosion Analysis</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              {[
                { label: 'Credit Cards', val: breakdown?.creditCardDebt, rate: '36-42%', color: 'border-accent' },
                { label: 'Formal Loans', val: breakdown?.formalLoans, rate: '8-12%', color: 'border-white/5' },
                { label: 'Private Debt', val: breakdown?.informalDebt, rate: '0-18%', color: 'border-white/5' }
              ].map((debt, i) => (
                <div key={i} className={`p-6 bg-white/[0.02] rounded-[24px] border ${debt.color} flex flex-col justify-between min-h-[140px] opacity-${debt.val > 0 ? '100' : '40 grayscale'}`}>
                  <div>
                    <div className="text-xs font-bold text-text2 mb-1">{debt.label}</div>
                    <div className="text-[9px] text-text3 font-black uppercase tracking-widest">{debt.rate} APR</div>
                  </div>
                  <div className="text-xl font-bold text-white font-mono">{formatCurrency(debt.val)}</div>
                </div>
              ))}
            </div>
            ...
            {totalLiabilities === 0 ? (
              <div className="p-10 bg-green/5 border border-green/10 rounded-3xl flex flex-col items-center justify-center text-center">
                ...
                <h5 className="font-extrabold text-white text-lg mb-2">Maximum Efficiency Detected</h5>
                <p className="text-xs text-text2 max-w-sm leading-relaxed">
                  Your architecture is 100% interest-efficient. You are currently losing ₹0 to financial institutions, allowing for maximum compound growth.
                </p>
              </div>
            ) : (
              <div className="flex items-center gap-3 p-4 bg-red/5 border border-red/10 rounded-2xl text-[11px] text-red font-medium">
                <Info size={14} />
                Strategic recommendation: Prioritize the liquidation of instruments with APR {`>`} 1.5% to improve efficiency.
              </div>
            )}
          </div>
        </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-10 relative z-10">
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-black text-white text-2xl tracking-tight">Q4 2026 Forecast</h4>
                      <p className="text-[11px] text-accent font-black uppercase tracking-[0.25em] mt-1">Growth Projection Engine</p>
                    </div>
                  </div>
                  
                  <div className="space-y-2">
                    <div className="text-[10px] font-black text-text3 uppercase tracking-[0.2em] opacity-60">Estimated Net Worth</div>
                ...
                <div className="flex-1 max-w-[320px] bg-white/5 backdrop-blur-xl p-6 rounded-3xl border border-white/10">
                  ...
                  >
                    Manage Portfolio <ArrowRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-12 mt-8 p-4 bg-white/[0.01] border border-white/5 rounded-2xl flex items-center justify-between pointer-events-none opacity-40">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-black uppercase tracking-[0.2em] text-text3">System Online</span>
            </div>
            <div className="w-[1px] h-3 bg-white/10"></div>
            <div className="text-[9px] font-mono text-text3 uppercase flex items-center gap-2">
              <span className="text-accent">$</span> architect_neural_engine --v1.0.4 --live_analysis
            </div>
          </div>
