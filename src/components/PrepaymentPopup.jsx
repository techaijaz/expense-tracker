...
export default function PrepaymentPopup({ open, setOpen, loanId, loanName, outstanding, onPaid }) {
  ...
  const handlePrepay = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Failed to process prepayment');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="modal-overlay">
      ...
      <div className="modal modal-sm" style={{ zIndex: 11 }}>
        ...
        <div className="modal-sub">{loanName} · Outstanding {formatAmount(outstanding, currency, decimalPlaces)}</div>
        ...
        <div style={{ background: 'var(--accent-glow)', border: '1px solid rgba(91,141,239,0.2)', borderRadius: 'var(--r)', padding: '16px', margin: '16px 0' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div style={{ textAlign: 'center', padding: '12px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--r2)' }}>
              <div style={{ fontSize: '18px', fontWeight: 700, fontFamily: 'var(--mono)', color: 'var(--green)' }}>
                {simulating ? '...' : simulation?.emisSaved || 0}
              </div>
              <div style={{ fontSize: '10px', color: 'var(--text2)', marginTop: '3px' }}>EMIs saved</div>
            </div>
            <div style={{ textAlign: 'center', padding: '12px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--r2)' }}>
              <div style={{ fontSize: '18px', fontWeight: 700, fontFamily: 'var(--mono)', color: 'var(--green)' }}>
                {simulating ? '...' : formatAmount(simulation?.interestSaved || 0, currency, decimalPlaces)}
              </div>
              <div style={{ fontSize: '10px', color: 'var(--text2)', marginTop: '3px' }}>Interest saved</div>
            </div>
            <div style={{ textAlign: 'center', padding: '12px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--r2)' }}>
              ...
              <div style={{ fontSize: '10px', color: 'var(--text2)', marginTop: '3px' }}>New outstanding</div>
            </div>
            <div style={{ textAlign: 'center', padding: '12px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--r2)' }}>
              <div style={{ fontSize: '18px', fontWeight: 700, fontFamily: 'var(--mono)', color: 'var(--amber)' }}>
                {simulating ? '...' : simulation?.newTenureMonths || '—'}
              </div>
              <div style={{ fontSize: '10px', color: 'var(--text2)', marginTop: '3px' }}>EMIs remaining</div>
            </div>
          </div>
        </div>
