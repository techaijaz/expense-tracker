...
const AddRecurringPopup = ({ open, setOpen, onSuccess, editTask = null }) => {
    ...
    if (open) {
      if (editTask) {
        reset({
          ...
          startDate: editTask.startDate ? new Date(editTask.startDate) : new Date(),
  ...
  const selectedAccount = accounts.find(a => (a._id || a.id) === selectedAccountId);
  const accountBalance = selectedAccount ? Number(selectedAccount.balance || 0) : 0;

  const onSubmit = async (data) => {
    // Balance Validation
    if ((data.type === 'EXPENSE' || data.type === 'TRANSFER') && data.amount > accountBalance) {
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/80 backdrop-blur-md px-4">
      <div className="relative w-full max-w-[520px] rounded-[24px] border border-white/10 bg-[#0E1220] p-7 shadow-2xl overflow-y-auto max-h-[90vh] hide-scrollbar">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Popover open={dateOpen} onOpenChange={setDateOpen}>
                    ...
                    <PopoverContent className="w-auto p-0 bg-[#0E1220] border-white/10" align="start">
                      <Calendar
                        mode="single"
            ...
            <div>
              <div className="flex items-center justify-between mb-2 ml-1">
                <label className="block text-[10px] font-bold text-[#4A5578] uppercase tracking-[0.1em]">Category</label>
                <button
                  type="button"
            ...
            <div>
              <label className="block text-[10px] font-bold text-[#4A5578] uppercase tracking-[0.1em] mb-2 ml-1">
                {selectedType === 'TRANSFER' ? 'Source Account' : selectedType === 'INCOME' ? 'Credit Account' : 'Debit Account'}
              </label>
              <select
                {...register('accountId')}
                className="w-full h-11 bg-[#141928] border border-white/10 rounded-xl px-3 text-sm text-[#EEF0F8] outline-none focus:border-[#5B8DEF]/50 cursor-pointer"
            ...
            {/* Frequency */}
            <div className={cn(selectedType !== 'TRANSFER' ? 'col-span-1' : 'col-span-2')}>
          ...
          <div>
            <label className="block text-[10px] font-bold text-[#4A5578] uppercase tracking-[0.1em] mb-2 ml-1">
              Remark / Note <span className="float-right lowercase font-normal opacity-50">{watch('notes')?.length || 0}/250</span>
            </label>
            <textarea
              {...register('notes')}
              maxLength={250}
            ...
            />
            {errors.notes && <p className="text-[11px] text-[#FF6B6B] mt-1">{errors.notes.message}</p>}
          </div>
