...
const assetSchema = z.object({
  ...
  currentValue: z.string().refine(val => !isNaN(parseFloat(val)) && parseFloat(val) > 0, 'Enter a valid market value'),
  initialValue: z.string().optional(),
  acquiredAt: z.date().max(new Date(new Date().setHours(23, 59, 59, 999)), 'Date cannot be in the future'),
  ...
  useEffect(() => {
    if (assetToEdit) {
      setFormData({
        ...
        acquiredAt: assetToEdit.acquiredAt ? new Date(assetToEdit.acquiredAt) : new Date(),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        initialValue: formData.initialValue ? parseFloat(formData.initialValue) : parseFloat(formData.currentValue),
      };
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: '16px', paddingRight: '40px' }}>
          ...
          <div style={{ fontSize: '11px', color: 'var(--text3)', marginTop: '4px', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            <span style={{ fontSize: '18px', fontWeight: 700, color: errors.currentValue ? '#ef4444' : 'var(--text3)', fontFamily: 'var(--mono)' }}>₹</span>
            <input
              type="text"
              inputMode="decimal"
              placeholder="0.00"
          ...
          {errors.currentValue && <div style={{ fontSize: '10px', color: '#ef4444', fontWeight: 600, marginBottom: '8px' }}>{errors.currentValue}</div>}
          ...
          >
            <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text3)', fontFamily: 'var(--mono)' }}>₹</span>
            <input
              type="text"
              inputMode="decimal"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
            ...
            <div>
                ...
                <PopoverTrigger asChild>
                  <Button
                    ...
                    style={{
                      width: '100%',
                      background: 'var(--bg3)',
                      border: errors.acquiredAt ? '1px solid rgba(239, 68, 68, 0.5)' : '1px solid var(--border2)',
                      borderRadius: 'var(--r2)',
                      color: formData.acquiredAt ? 'var(--text)' : 'var(--text2)',
                      fontSize: '12px',
                    }}
                  >
                    <CalendarIcon className="mr-2 h-3 w-3 opacity-50" />
                    {formData.acquiredAt instanceof Date ? format(formData.acquiredAt, 'dd MMM yyyy') : 'Pick a date'}
                  </Button>
                </PopoverTrigger>
              ...
              {errors.acquiredAt && <div style={{ fontSize: '10px', color: '#ef4444', fontWeight: 600, marginTop: '4px' }}>{errors.acquiredAt}</div>}
            </div>
          </div>
          ...
          <div style={{ marginBottom: '8px' }}>
            ...
            <input
              ...
              style={{ 
                ...inputStyle, 
                padding: '8px 12px',
                border: errors.name ? '1px solid rgba(239, 68, 68, 0.5)' : '1px solid var(--border2)' 
              }}
            />
            {errors.name && <div style={{ fontSize: '10px', color: '#ef4444', fontWeight: 600, marginTop: '4px' }}>{errors.name}</div>}
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '12px', justifyContent: 'flex-end' }}>
            ...
            <button
              ...
              style={{
                padding: '8px 24px',
                background: (loading || Object.keys(errors).length > 0) ? 'var(--bg4)' : 'var(--accent)',
                border: 'none',
                borderRadius: 'var(--r2)',
                color: '#fff',
                fontFamily: 'var(--font)',
            ...
            >
              ...
              {loading ? 'Processing…' : (assetToEdit ? 'Update Valuation' : 'Commit Asset')}
            </button>
          </div>
        </form>
