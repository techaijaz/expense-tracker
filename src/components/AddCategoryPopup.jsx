...
const TYPE_CONFIG = {
  INCOME:   { label: 'Income',   icon: 'trending_up',   color: '#a8edca', bg: 'rgba(168,237,202,0.1)',  border: 'rgba(168,237,202,0.3)' },
  EXPENSE:  { label: 'Expense',  icon: 'shopping_cart', color: '#f97171', bg: 'rgba(249,113,113,0.1)', border: 'rgba(249,113,113,0.3)' },
...
const AddCategoryPopup = ({ open, onClose, onSave, editCategory = null, defaultType = 'EXPENSE' }) => {
  ...
  const onSubmit = (formData) => {
    ...
    if (editCategory) {
      makeRequest({ url: `/catagory/${editCategory._id}`, method: 'patch', data: payload });
    } else {
  ...
  const inputClass = (field) => `w-full py-[11px] px-3.5 rounded-[10px] text-on-surface text-sm outline-none transition-all duration-200 font-body ${
    focusedField === field 
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[460px] bg-card border border-secondary-container rounded-[20px] py-8 px-[30px] shadow-[0_40px_80px_rgba(0,0,0,0.6)]">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          {/* Type selector */}
          <div className="mb-5">
            <label className="block text-[11px] font-bold text-on-surface-variant uppercase tracking-[0.08em] mb-2.5">Category Type</label>
            <div className="grid grid-cols-3 gap-2">
              {Object.entries(TYPE_CONFIG).map(([key, conf]) => (
                <label
                  ...
                  style={{
                    backgroundColor: selectedType === key ? conf.bg : 'var(--input-bg)',
                    borderColor: selectedType === key ? conf.border : 'var(--input-bg)'
                  }}
                >
                  <input type="radio" {...register('type')} value={key} className="hidden" />
                  <span className="material-symbols-outlined text-[20px]" style={{ color: selectedType === key ? conf.color : 'var(--text-muted)', fontVariationSettings: "'FILL' 0" }}>{conf.icon}</span>
                  <span className="text-[11px] font-bold tracking-[0.04em]" style={{ color: selectedType === key ? conf.color : 'var(--text-muted)' }}>{conf.label}</span>
                </label>
              ))}
            </div>
            {errors.type && <p className="mt-1.5 text-[11px] text-error">{errors.type.message}</p>}
          </div>

          {/* Name */}
          <div className="mb-5">
            <label className="block text-[11px] font-bold text-on-surface-variant uppercase tracking-[0.08em] mb-2">Category Name</label>
            <input
              type="text"
              {...register('name')}
          ...
          <div className="mb-6">
            <label className="block text-[11px] font-bold text-on-surface-variant uppercase tracking-[0.08em] mb-2">
              Icon &nbsp;<span className="text-on-surface-variant font-normal normal-case">Selected: {selectedIcon}</span>
            </label>
          ...
          <div className="flex gap-2.5">
            <button type="button" onClick={onClose} className="flex-1 p-3 bg-secondary-container border border-secondary-container rounded-[10px] text-on-surface-variant text-[13px] font-semibold cursor-pointer">
              Cancel
            </button>
            <button
