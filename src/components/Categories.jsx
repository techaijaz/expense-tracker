      ...
      >
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 min-w-0">
            <div className={`w-12 h-12 rounded-lg ${config.bg} ${config.color} flex items-center justify-center text-2xl shadow-inner border border-white/5`}>
              {cat.icon || '🏷️'}
            </div>
            <div className="min-w-0">
              ...
              <p className={`text-[10px] font-black uppercase tracking-widest ${config.color} mt-0.5 opacity-80`}>
                {cat.type}
              </p>
            </div>
          </div>
          ...
          {[1, 2, 3].map((i) => (
            <div key={i} className="space-y-6">
              ...
              <div className="space-y-4">
                {[1, 2, 3].map((j) => (
                  <div key={j} className="h-20 bg-surface-container-highest/20 rounded-xl animate-pulse" />
                ))}
              </div>
            </div>
          ))}
        ...
        <DialogContent className="sm:max-w-[425px] bg-surface-container-highest border-outline-variant/20 shadow-2xl">
          <DialogHeader>
            ...
            <DialogDescription className="py-4 text-outline font-medium text-sm leading-relaxed">
              Deleting <span className="text-on-surface font-bold">"{deletingCategory?.name}"</span> will result in its transactions being orphaned or reassigned to "General". This cannot be undone.
            </DialogDescription>
          </DialogHeader>
