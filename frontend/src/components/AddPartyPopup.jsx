  ...
  return (
    ...
    >
      ...
      <div className="relative z-[1] w-full max-w-[400px] bg-surface border border-input-bg rounded-[20px] p-7">
        <h3 className="text-base font-extrabold text-on-surface mb-5">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {['FRIEND', 'FAMILY', 'VENDOR', 'CLIENT'].map((r) => (
              <label
                key={r}
                className={`flex items-center justify-center px-3 py-2.5 rounded-xl cursor-pointer border-2 transition-all duration-200 ${relation === r ? 'bg-primary/10 border-primary text-primary' : 'bg-surface-container-low border-transparent text-on-surface-variant hover:border-outline-variant/30'}`}
              >
                <input
                  type="radio"
                  name="relation"
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={onClose}
            className="flex-1 h-11 bg-surface-container-high hover:bg-surface-container-highest text-on-surface rounded-xl text-sm font-bold transition-all"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={busy}
            className={`flex-[1.5] h-11 bg-primary text-background rounded-xl text-sm font-bold shadow-lg shadow-primary/20 hover:opacity-90 active:scale-[0.98] transition-all ${busy ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
