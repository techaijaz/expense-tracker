...
const TAB_CONFIG = {
  expense: {
    label: '💸 Expense',
    activeColor: 'var(--red)',
    textClass: 'tab-expense',
  },
  income: {
    label: '💵 Income',
    activeColor: 'var(--green)',
    textClass: 'tab-income',
  },
  transfer: {
    label: '⇄ Transfer',
    activeColor: 'var(--accent)',
    textClass: 'tab-transfer',
  },
  debt: {
    label: '🤝 Debt',
...
const TransactionPopup = ({
    ...
    if (open) {
      if (editTransection) {
        reset({
          ...
          account:
            editTransection.accountId?._id || editTransection.accountId || '',
          description: editTransection.title || editTransection.notes || '',
          category:
            editTransection.categoryId?._id || editTransection.categoryId || '',
          toaccount:
            editTransection.targetAccountId?._id ||
            editTransection.targetAccountId ||
            '',
          debttype: editTransection.debtType || '',
          partyId:
            editTransection.partyId?._id || editTransection.partyId || '',
          dueDate: editTransection.dueDate
            ? new Date(editTransection.dueDate)
            : null,
  ...
  const onSubmit = async (data) => {
    ...
    if (data.type === 'debt' && data.loanId) {
      ...
      if (selectedLoan && data.amount > selectedLoan.amount) {
        toast.error(
          `Repayment cannot exceed outstanding balance (${formatAmount(selectedLoan.amount)})`,
        );
        return;
      }
    }
    ...
    try {
      ...
      const payload = {
        ...
        type:
          data.type === 'debt' && data.debttype === 'repay'
            ? 'repayment'
            : data.type,
        amount: data.amount,
      ...
      if (['debt', 'repayment'].includes(payload.type)) {
        ...
        payload.debtType =
          data.debttype === 'repay' ? 'REPAY_IN' : data.debttype?.toUpperCase();
      ...
      toast.success(
        `Transaction ${editTransection ? 'updated' : 'added'} successfully!`,
      );
  ...
  const activeTabColor =
    TAB_CONFIG[transactionType]?.activeColor || 'var(--accent)';
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            >
                  ...
                  render={({ field }) => (
                    <Popover open={dateOpen} onOpenChange={setDateOpen}>
                      <PopoverTrigger asChild>
                        ...
                        >
                          <span className="material-symbols-outlined mr-2 !text-[16px] opacity-70">
                            calendar_today
                          </span>
              ...
              {transactionType === 'debt' ? (
                <div>
                  ...
                  {errors.partyId && (
                    <p className="error-msg">{errors.partyId.message}</p>
                  )}
                </div>
            ...
            >
              ...
              <div>
                ...
                {errors.account && (
                  <p className="error-msg">{errors.account.message}</p>
                )}
              </div>
              ...
              {transactionType === 'transfer' ? (
                <div>
                  ...
                  {errors.toaccount && (
                    <p className="error-msg">{errors.toaccount.message}</p>
                  )}
                </div>
              ) : isRepayment ? (
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (loan) {
                        ...
                        setValue(
                          'description',
                          `Repayment of ${loan.type === 'LENT' ? 'receivable' : 'payable'} from ${loan.party?.name}`,
                        );
                      }
                    }}
                    style={{
                      ...inputStyle,
                      border: errors.loanId
                        ? '1px solid var(--red)'
                  ...
                  {errors.loanId && (
                    <p className="error-msg">{errors.loanId.message}</p>
                  )}
              ...
              >
                {isRestrictedEdit && (
                  ...
                  >
                    <div>
                      ...
                      >
                        Editing Take/Give debt is restricted.
                      </p>
                      <p style={{ fontSize: '11px', color: 'var(--text2)' }}>
                        You can only edit repayment transactions here.
                      </p>
                    </div>
                  </div>
                )}
                ...
                >
                  ...
                  <div>
                    ...
                    />
                    {errors.dueDate && (
                      <p className="error-msg">{errors.dueDate.message}</p>
                    )}
                  </div>
